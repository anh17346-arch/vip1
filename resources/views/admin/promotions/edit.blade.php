...
@section('title', 'Chỉnh sửa khuyến mãi - Perfume Luxury')
  ...
  <div class="relative max-w-4xl mx-auto px-4 py-8">
    ...
    <div class="mb-8">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-3xl font-bold text-slate-900 dark:text-slate-100">Chỉnh sửa khuyến mãi</h1>
          <p class="text-slate-600 dark:text-slate-400 mt-2">Cập nhật thông tin khuyến mãi "{{ $promotion->name }}"</p>
        </div>
    ...
    <div class="backdrop-blur-md bg-white/20 dark:bg-white/5 rounded-2xl shadow-lg border border-white/30 dark:border-white/10 overflow-hidden">
      <form action="{{ route('admin.promotions.update', $promotion) }}" method="POST" class="p-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="space-y-6">
            ...
            <div>
              ...
              <input type="text" name="name_en" id="name_en" value="{{ old('name_en', $promotion->name_en) }}"
                     class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-slate-800 dark:text-slate-100">
              @error('name_en')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
              @enderror
            ...
            <div>
              ...
              <textarea name="description" id="description" rows="3"
                        class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-slate-800 dark:text-slate-100">{{ old('description', $promotion->description) }}</textarea>
              @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
              @enderror
            </div>
            ...
            <div>
              ...
              <textarea name="description_en" id="description_en" rows="3"
                        class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-slate-800 dark:text-slate-100">{{ old('description_en', $promotion->description_en) }}</textarea>
          ...
          <div class="space-y-6">
            ...
            <div>
              ...
              <input type="number" name="discount_value" id="discount_value" value="{{ old('discount_value', $promotion->discount_value) }}" 
            ...
            <div>
              ...
              <input type="number" name="min_order_amount" id="min_order_amount" value="{{ old('min_order_amount', $promotion->min_order_amount) }}" 
                     step="1000" min="0"
                     class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-slate-800 dark:text-slate-100">
            ...
            <div>
              ...
              <input type="number" name="max_discount_amount" id="max_discount_amount" value="{{ old('max_discount_amount', $promotion->max_discount_amount) }}" 
                     step="1000" min="0"
                     class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-slate-800 dark:text-slate-100">
        ...
        <div class="mt-8 space-y-6">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <select name="applies_to" id="applies_to" required
                ...
                <option value="specific_products" {{ old('applies_to', $promotion->applies_to) == 'specific_products' ? 'selected' : '' }}>Sản phẩm cụ thể</option>
              </select>
              @error('applies_to')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
              @enderror
            ...
            <div>
              ...
              <select name="user_type" id="user_type" required
                      class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-slate-800 dark:text-slate-100">
                <option value="all_users" {{ old('user_type', $promotion->user_type) == 'all_users' ? 'selected' : '' }}>Tất cả người dùng</option>
                <option value="new_users" {{ old('user_type', $promotion->user_type) == 'new_users' ? 'selected' : '' }}>Người dùng mới (≤ 30 ngày)</option>
                <option value="existing_users" {{ old('user_type', $promotion->user_type) == 'existing_users' ? 'selected' : '' }}>Người dùng cũ (> 30 ngày)</option>
              </select>
              @error('user_type')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
              @enderror
          ...
          <div id="categories-section" class="{{ old('applies_to', $promotion->applies_to) == 'specific_categories' ? '' : 'hidden' }}">
          ...
          <!-- Products Selection -->
          <div id="products-section" class="{{ old('applies_to', $promotion->applies_to) == 'specific_products' ? '' : 'hidden' }}">
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
              Chọn sản phẩm
            </label>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 max-h-48 overflow-y-auto p-3 border border-slate-300 dark:border-slate-600 rounded-lg">
        ...
        <div class="mt-8">
          <label class="flex items-center space-x-2 cursor-pointer">
            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $promotion->is_active) ? 'checked' : '' }}
                   class="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500">
            <span class="text-sm font-medium text-slate-700 dark:text-slate-300">Kích hoạt khuyến mãi</span>
          </label>
        </div>
